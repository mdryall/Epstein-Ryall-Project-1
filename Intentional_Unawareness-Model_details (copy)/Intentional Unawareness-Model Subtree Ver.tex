...
%Notice that states in $S^\oplus$ may appear in multiple  of the subtrees that represent $i$'s awareness. 
%That is, $s$ could be an element in each of $S^\ominus_{s^\prime}$, $S^\ominus_{s^{\prime\prime}}$, $S^\ominus_{s^{\prime\prime\prime}}$ and so on.
%In what follows, we will sometimes need to associate $s$ with the subjective version of reality in which it appears.
%Therefore, for each $s\in S^\oplus$, let us think of the states in $S^\ominus_s$ as distinct \textit{copies} of their counterparts in $S^\oplus$.
%Then, define $S\equiv \bigcup_{s\in S^\oplus}S^\ominus_s\cup S^\oplus$ as the union of \textit{all} these states: all of those corresponding to reality ($S^\oplus$) as well as all their copies.
...
%Given $s^\oplus_t$, the set of all \textit{subjective histories at time $t$} is $H^\ominus_t(s^\oplus_t)$, with an arbitrary element  denoted $h^\ominus_t$  and the set of all subjective complete histories denoted $H^\ominus_T(s^\oplus_t)$.
%Again, when  $s^\oplus_t$ is arbitrary or understood from the context, we drop the argument notation and write $H^\ominus_t$ and $H^\ominus_T$.
...
\begin{figure}[h!]
	...
	\caption{Infant is uncertain about which awareness state is factual\label{Diag: p-03}}%trim: L B R TxPer
\end{figure}
...
\begin{figure}[h!]
	...
	\caption{Infant is aware of everything but uncertain about the weather\label{Diag: p-04}}%trim: L B R TxPer
\end{figure}
...
\begin{figure}[h!]
	...
	\caption{Synchronic events in the Brian's Infant example\label{old1}}%trim: L B R TxPer
\end{figure}
