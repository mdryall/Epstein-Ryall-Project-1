...
%The idea is to map the objective, full-featured description of how the world may evolve according to $g^\oplus$ to impoverished versions that include only the features of the world of which individual $i$ is aware in any given objective state.
%Thus, if $i$ is in the objective state $s$, then  $s^\prime\in S_s$ represents $i$'s subjective awareness of a state in period $k$.
...
%	}
%	
%	Finally, given  $s^\ominus_k\in S(s^\oplus_t)$  the \textit{subjective history at state $s^\ominus_k$} is the path in $G(s^\oplus_t)$ that starts at $s^\ominus_0$ and ends at $s^\ominus_k$ and is given by $h(s^\ominus_k)$.
...
\begin{figure}[h!]
	...
	\caption{Fully aware child is uncertain about which toy is best\label{Diag: p-03}}%trim: L B R TxPer
\end{figure}
