...
\begin{document}
	...
	Let $S^0$ denote the set of all possible states of the world, delineated in their richest level of detail.
	The  ``0'' superscript indicates that this corresponds to Nature's ``perception'' of reality; i.e., states of reality as they may truly exist. 
	Individual superscripts, e.g., $S^i$, $i\ne 0$,  indicate individual $i$'s (limited) awareness of reality, which we explain in further detail below.
	For this analysis, we assume time is discreet and limit attention to some finite number of periods $T$. 
	...
	{
		This setup can be generalized to include countably or uncountably infinite state spaces. Limiting attention to finite sets allows us to sidestep some mathematical complexities which would add little to our analysis.
	} 
	...
	The term `event' is used differently by philosophers than it is by game theorists. 
	Since we are writing to both audiences, it is important to clarify this difference. 
	...
	We refer to events associated with states at a moment in time (the game theory useage) as \textit{synchronic}, and those associated with states unfolding through time (the philosophy useage) as \textit{diachronic}. 
	
	A \textit{synchronic event} is a subset of a state space at a specific time.  
	Because an individual state encodes all relevant features of the world, synchronic events provide a way of identifying all the states consistent with a particular feature of interest.
	For example, in our context, the event ``Mike intends to get a cup of coffee in period $t$''  includes \textit{all} states in which getting a cup of coffee is the intention of Mike in period $t$. 
	...
	Let $\Sigma^0_t\equiv 2^{S^0_t}$ be the set of all  \textit{synchronic events in period $t$}, with typical element $\sigma^0_t\in \Sigma^0_t$.
...
As discussed earlier, $S^0_t$ is the richest state space -- each element would be associated with a God's-eye awareness of all aspects of reality at time $t$.
According to this logic, $s^0_t\in S^0_t$ includes the mental states of individual $i$.
Therefore, define the \textit{projection} $r^{0\rightarrow i}_t:S^0_t\rightarrow S^i_t$, where  $S^i_t$ is a state space whose elements include only the features of reality of which $i$ is aware and $s^i_t=r^{0\rightarrow i}_t(s^0_t)$ is the impoverished version of reality $i$ knows when $s^0_t$ is the fully expressed state of the world. 
In other words, $s^i_t$ includes all the features of reality that an individual can bring to mind and relate to a decision at hand.
