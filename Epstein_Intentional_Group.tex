...
\begin{document}
	...
	We account for natural phenomena as the ``actions'' of a ``Nature'' which we assign to population index 0.
	...
	Let $S$ denote the set of all possible states of the world.
	For our analysis, we assume time is discreet and limit attention to some finite number of periods $T$. 
	The finite \textit{state space at time} $t$, denoted $S_t\subset S$, with typical element $s_t\in S_t$, contains all the states that could possibly be actualized at $t$.\footnote
	{
	...
	We will use the game-theorist's meaning of the term, but since we are writing to both audiences, it is important to clarify this difference. 
	...
	We will distinguish between events associated with states at a moment in time, which we refer to as \textit{synchronic}, and those associated with states unfolding through time. 
	A \textit{synchronic event} is a subset of $S$.  
	Let $\mathcal{S}\equiv 2^S$ be the set of all possible \textit{events}, with typical element $E\in \mathcal{S}$.
	We will define diachronic events momentarily.
	...
	Here, for example, we can let $d(s)$ indicate the outcome of a die roll in state $s$.
	Given $s\in S$, $d(s)\in\{1,2,3,4,5,6\}$, assuming the die never lands on an edge. 
	To clarify the domain and the range to which the function maps, we write $d:S\rightarrow \{1,2,3,4,5,6\}$; i.e., $d$ maps from a state in $S$ to a number between 1 and 6, inclusive.
	...
	For all individuals $i\in N$,  $A^i(s_t)$ indicates the set of \textit{feasible acts available to individual $i$ in state $s_t$} with typical element $a^i_t\in A^i(s_t)$.\footnote
	{
		Notice that we use a capital letter to indicate that $A^i(s_t)$ is a set-valued function: $A^i\rightarrow 2^A$.
		Also note that we use superscripts for individual's indices and subscripts to index time.
	...
	The set of \textit{all act profiles at state $s_t$} is $\mathbf{A}(s_t)\equiv \times_{i=0}^n A^i(s_t)$. 
	The set of \textit{all possible act profiles} is $\mathbf{A}\equiv \cup_{s\in S} \mathbf{A}(s)$. 
	
	\subsection{Dynamics  } 
	  
	As indicated above, the act profiles summarize all the contingencies required to actualize one state from the next. 
	To formalize this, let $\omega:S\times \mathbf{A}\rightarrow S$ be the \textit{state-contingent actualization function}, where $\omega(\mathbf{a}_t|s_t)=s_{t+1}$ indicates that if the act profile at state $s_t\in S_{t}$ is $\mathbf{a}_t\in \mathbf{A}(s_t)$, then the next state actualized is $s_{t+1}$.
	Assume that, for all $s_t$, $\omega:\{s_t\}\times \mathbf{A}_t(s_t)\rightarrow \cup_{\mathbf{a}_t\in \mathbf{A}_t(s_t)} \omega(\mathbf{a}_t|s_t)$ is a bijection.
	...
	That is, $\mathbf{a}_{0}=(a^0_0,\emptyset,\ldots,\emptyset)$, where $a^0_0$ represents all the actualize historic factors that lead individuals to their  first decision state, $s_1=\omega(\mathbf{a}_0|s_0)$.
	...
	We define the \textit{history at state $s_t$} as a profile that starts at $s_0$ and ends at $s_t$ along with the  acts that determine the sequence of states according to $\omega$, denoted $\mathbf{h}(s_t)=(s_0,a_0,\ldots,a_{t-1},s_t)$, where $s_1=\omega(a_0|s_0)$, etc.
	The set of all histories at time $t$ is $\mathbf{H}_t$.
	The set of all possible histories is $\mathbf{H}_T$ and the set of all subsets of histories is $\mathcal{H}_T$.
	An arbitrary \textit{history at time $t$} is denoted $\mathbf{h}_t\in \mathbf{H}_t$, where we start with the \textit{null history} $\mathbf{h}_0=(s_0)$ at the beginning of time (so, $\mathbf{H}_0=\{\mathbf{h}_0\}$ and  $S_0=\{s_0\}$).   
	...
	A \textit{diachronic event} is a subset $E\in 2^{\mathbf{H}_T}$; i.e., a subset of paths in the tree associated with $\mathbf{H}_T$.
	Let $\mathcal{H}\equiv 2^{\mathbf{H}_T}$ be the set of all diachronic events.
	Given the preceding discussion, every event $E\in\mathcal{S}$ is associated with a unique event $E^\prime\in \mathcal{H}$. 
...
In each period $t$, we assume that every individual is associated with a state space representing the states of which that individual is aware. 
...
As we will elaborate below, $S_t$ is the richest space -- it is a God's-eye perception of all relevant aspects of reality at time $t$.
Then, $S^i_t$ is a state space that represents the aspects of $S_t$ of which individual $i$ is aware. 
Note that $t$ may be a future state -- the future possibilities that an individual can bring to mind and relate to a decision at hand.
...
Importantly, awareness may extend to the mental states of others, for example, whether the leather-clad man in the biker bar intends to strike. 
 We adopt the intuitive convention that $S^0_t=S_t$.
We also extend the set of awareness spaces $\mathcal{S}_t$ to include a ``null space,'' denoted $S_\emptyset=\{s_\emptyset\}\in\mathcal{S}_t$, where $s_\emptyset$ represents a state of complete unawareness of anything. 

We assume that the state spaces in $\mathcal{S}_t$ are partially ordered according to their richness relative to $S_t$. 
Mathematically, let $\langle \mathcal{S}_t, \succeq\rangle$ be a bounded lattice of spaces ($\succeq$ is a partial order on the spaces in  $\mathcal{S}_t$), such that $S^k_t\succeq S^i_t$ means ``$S^k_t$ is a (weakly) richer description of the world than $S^i_t$.''
The richest state space is $S^0_t$ (a maximum of the lattice), and the poorest is $S_\emptyset$ (a minimum of the lattice).
To keep track of the interrelationships between states, we introduce projections to map states in richer spaces to their counterparts in poorer spaces.
...
\begin{enumerate} 
	...
	\item Some states in a richer $S^k_t$ may have no counterpart in $S^i_t$.
\end{enumerate}
...
Assume individual $i$ is only aware that he may be told ``blue'' or ``green''. 
Then, $S^i_1=\{blue, green\}$ and $r^{0\to i}_1(3)=r^i_1(4)=blue$ and $r^{0\to i}_1(5)=r^i_1(6)=green$.
Notice that: i) $r^{0\to i}$ is not defined on 1 or 2; and ii) the states in $S^i_1$ correspond to events in $S^0_1$.
...
It is also useful to have notation for all states in an awareness structure, no matter in which space: let $\Omega = S^0_t \cup S' \ldots \cup S^n_t$ denote the \textit{union of spaces} in $\mathcal{S}_t$.
...
If $E_t\subseteq S^0_t$, then we denote by $[E]_t\subseteq \Omega_t$ the states that correspond to $E_t$ in the awareness structure $\mathcal{S}_t$.
